<x-layouts.base>
    ...
    <main class="content">
        ...
        <div class="p-2 mb-0 mt-2">
            <div class="row">
                <div class="col-12 col-md-4 col-xl-6">
                    <p class="mb-0 text-center text-lg-start"><b class="">Update Parent</b></p>
                    <p class="mb-0 text-center text-lg-start"><small class="">Home - Users - Update Parent Info</small>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded p-4 mb-4 mt-2">
            ...
            <h2 class="h5 mb-4">Update Parent information</h2>
            <form action="{{ route('admin.parent.update', $parent->id) }}" method="POST"
                            ...
                            <select name="class_id" class="form-select eChoice-multiple-with-remove" required
                                ...
                                @foreach ($classes as $class)
                                    <option value="{{ $class->id }}"
                                        {{ $parent->class_id == $class->id ? 'selected' : '' }}>{{ $class->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="section_id" class="form-select eChoice-multiple-with-remove" required>
                                ...
                                @foreach ($sections as $section)
                                    <option value="{{ $section->id }}"
                                        {{ $parent->section_id == $section->id ? 'selected' : '' }}>{{ $section->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="mt-3">
                    <button type="submit" class="btn btn-gray-800 mt-2 animate-up-2">Update Parent Info</button>
                </div>
            </form>
        </div>
        </div>
