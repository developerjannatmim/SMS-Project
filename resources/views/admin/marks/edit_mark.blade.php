    ...
    <section class="section" style="margin-top: -120px">
      ...
      <div class="bg-white rounded p-4 mb-4 mt-2">
        <form action="{{ route('admin.marks.update', $mark->id ) }}" method="POST">
            ...
            <div class="col-md-6 mb-3">
              <div>
                ...
                <select name="user_id" class="form-select eChoice-multiple-with-remove" required>
                  ...
                  <option value="{{ $student_name->id }}" {{ $mark->user_id == $student_name->id ? 'selected':'' }}>{{ $student_name->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="col-md-6 mb-3">
              <div class="form-group">
                ...
                <select name="class_id" class="form-select eChoice-multiple-with-remove" required>
                  ...
                  <option value="{{ $class->id }}" {{ $mark->class_id == $class->id ? 'selected':'' }}>{{ $class->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
              <div class="form-group">
                ...
                <select name="section_id" class="form-select eChoice-multiple-with-remove" required>
                  ...
                  <option value="{{ $section->id }}" {{ $mark->section_id == $section->id ? 'selected':'' }}>{{ $section->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="col-md-6 mb-3">
              <div>
                ...
                <select name="exam_id" class="form-select eChoice-multiple-with-remove" required>
                  ...
                  <option value="{{ $exam->id }}" {{ $mark->exam_id == $exam->id ? 'selected':'' }}>{{ $exam->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
          ...
          <div class="mt-3">
            <button type="submit" class="btn btn-primary mt-2 animate-up-2">Update</button>
          </div>
        </form>
      </div>
      </div>
